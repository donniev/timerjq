	...
	$.fn.extend({
  		jTimer:function(method,_options){
			...
			var initCallback=function(me){
				if(typeof (options.callback)==='function'){
					options.callbackHandle=setInterval(function(){
						me.data("pluginoptions").callback(me.jTimer("getElapsedTime"));
					},options.callbackInterval);
					
				}
			}
			var timerGuts = function(me){
				var options=me.data("pluginoptions");
			...
			if(method=="getElapsedTime"){
				return Math.round(this.data("pluginoptions").timerVals.elapsedTime/1000);
				}
			return this.each(function(){
				...
				if(method=="remove"){
					me.children(".timerDisplay,.timerButtonGroup").remove();
					clearInterval(me.data("pluginoptions").timerVals.timerHandle);
					clearInterval(me.data("pluginoptions").callbackHandle);
				}
